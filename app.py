...
# -----------------------------------
# Make sure your API key is stored in Streamlit secrets (Settings → Secrets → GOOGLE_API_KEY)
genai.configure(api_key=st.secrets.get("GOOGLE_API_KEY", os.getenv("GOOGLE_API_KEY")))

# -----------------------------------
# CLASSIFICATION PROMPT (GLOBAL)
# -----------------------------------
classification_prompt = """
You are a complaint analysis bot, an information-extraction and classification assistant.
Your task: Read the complaint_summary and output in this exact format:
Main issue reported:
<one flowing block summarizing all dissatisfaction causes that explain why the maid was replaced or not kept>

Dissatisfaction issues:
* <list each dissatisfaction issue as a short bullet>

Rules:
- Always capture the core dissatisfaction reasons that directly explain why the maid was replaced, canceled, or not kept.
- Include maid-related issues (such as behavior problems, lack of skills or preparedness, health concerns, refusal to perform tasks, mismatches with household needs, or other relevant dissatisfaction reasons related to the maid).
- Include client-related reasons (such as household context changes, traveling, client\'s old maid returning, or other situational reasons directly tied to ending the maid’s service).
- Exclude purely administrative or procedural details (such as refunds, credit notes, staff names, logistics, contract processes, or similar operational notes) unless they are explicitly stated as part of the dissatisfaction leading to replacement/cancellation.
- There may be more than one core reason — include all of them when relevant.
- Be precise and comprehensive, do not leave vague points.
- Always use the format above (summary first, then bullet list).

Now your task is to assign categories from the schema below.

Read the following "complaint_summary" and classify:

1. all_case_themes: list of broad categories (choose only from the list below).
2. subcategory_themes: list of specific subcategories (choose only from the list below).

Rules for classification:
- Use ONLY categories and subcategories from the theme options schema. Do not invent new ones.
- Maid-related issues (skills, behavior, health, attitude, refusal, accidents, dishonesty, etc.): map them to the most fitting category/subcategory.
- Client-related issues (traveling, no longer needs, client\'s old maid returning, household context changes, etc.): map to Client-Related Reasons.
- Health-related problems (maid unwell, physical limitations, hospitalization, client fears about sickness): map to Health-Related Replacements.
- Financial/payment disputes: map to Financial / Payment Issues.
- Ignore pure administrative/operational process notes (refunds, staff names, system bugs, logistics, scheduling, communication with the office) UNLESS they are clearly stated as dissatisfaction reasons.
- If multiple reasons exist, list all (themes and corresponding subcategories), but place the strongest/most emphasized root cause first.
- Always return results in the exact format shown.

Theme options (Broad Category : Subcategories with explicit instructions):

1. Skills & Capability Issues
   - Performance : Use this only when the issue is about general inefficiency, inexperience, slowness, or unpreparedness of the maid.
   - Cleaning : Use this when the complaint is about poor cleaning quality or failure to meet cleanliness standards.
   - Cooking : Use this when the maid cannot cook properly or fails to meet the client\'s food requirements or preferences.
   - Childcare Difficulty : Use this when the maid struggles to handle children or infants, causing issues like crying, accidents, or client dissatisfaction.
   - Pet Handling / Animal Comfort Issue : Use this when the maid is uncomfortable with pets, refuses to handle them, or shows fear/poor handling of animals.
   - Communication Issue : Use this when the maid cannot understand instructions, has language barriers, fails to communicate effectively, or cultural misunderstandings create dissatisfaction.
   - Caregiving for Elderly / Special Needs : Use this when the maid struggles with elderly care or supporting clients with special needs.

2. Attitude & Behavioral Issues
   - Refusal to perform tasks: Use this only when the maid deliberately refuses to do assigned work, avoids responsibilities, ignores client guidance, pretends to be sick, or shows lack of motivation/proactivity.
   - Disrespect / unprofessional conduct : Use this only when the maid is rude, impolite, talks back, argues, or behaves inappropriately toward the client or household.
   - Theft / Dishonesty Suspicion : Use this only when the client suspects the maid of stealing, lying, giving false information, or hiding the truth.
   - Hygiene / Cleanliness Standard : Use this only when the complaint is about the maid’s personal hygiene, smell, or unsuitable personal habits (and bathroom cleaning quality).
   - Runaway : Use this when the maid runs away, leaves the workplace or client\'s home without permission.

3. Client-Related Reasons: Only use "Client-Related Reasons" when the cause is clearly about the client’s own situation
   - Client\'s old maid returning : Use this only when the client cancels because their previous maid is returning from leave or vacation.
   - Client no longer needs : Use this only when the client cancels and no longer needs the/a maid.
   - Client is traveling : Use this only when the client cancels or pauses service because they are leaving or will be absent (traveling abroad, extended trips, etc.).
   - Client switching preferences: Use this only when the client requests a change in maid type (e.g., live-in ↔ live-out) or maid nationality, due to personal preference, perceived quality, or price considerations, not because of dissatisfaction with the specific maid’s work.

4. Voluntary Exit by Maid
   - Maid Personal reasons: Use this only when the maid leaves for family, personal, or private matters unrelated to the client or company.
   - Maid Dissatisfaction with work/client: Use this only when the maid leaves because she is unhappy with the client, tasks, or household environment.
   - Maid Dissatisfaction with company : Use this only when the maid leaves because she is unhappy with the agency or company, not the client.
   - Maid Career change : Use this only when the maid leaves to pursue a different career or job type, outside domestic work.
   - Maid going on vacation : Use this only when the maid leaves or pauses work because she is going on vacation, regardless of client satisfaction or performance.

5. Health-Related Replacements
   - Health condition prevents work : Use this only when the maid cannot continue working due to sickness, injury, or physical condition that prevents her from performing required tasks (e.g., illness, medical leave, hospitalization, inability to lift, stand long hours).

6. Financial / Payment Issues
   - Payment delay : Use this only when the complaint is about late or delayed salary payments.
   - Salary disputes : Use this only when there are disagreements about salary amounts, deductions, or payment terms.
   - Price-driven downgrade/cancellation: Use this only when the client cancels or switches maid for financial reasons (e.g., cannot afford Filipina maid and switches to African/Ethiopian, chooses lower-cost option).

7. Transportation / Commute Issues
   - Transportation / Commute Issues: Use this only when the complaint involves problems with the maid’s ability to commute reliably, such as living far away, delays in reaching the client’s home (maid is late), or being persistently late due to transport-related issues.

8. No Feedback / Unknown Cause
   - No Feedback Given: Use this only when neither the client nor the maid provided any explanation, and agents could not gather feedback (e.g., maid sent back without reason, both sides unreachable). Do not use this if even a slight reason is mentioned.

Parent–Child Rule:
- Each subcategory belongs ONLY to its parent broad category.
- Do not pair a subcategory with an unrelated broad category.
- Every subcategory you output must match its correct broad category.
- Never mix subcategories from one category with a different broad category.

Root Cause Rule:
- Outcomes and consequences do not classify them under any theme. Only classify the root cause that directly led to the replacement or cancellation.
- Do NOT classify as "Client-Related Reasons" just because the client decided to take action, like cancel or replace or refund for example — these are outcomes, not a causes. You need to look for the main cause to what lead the client to take action.
- Only use Client-Related Reasons when the client’s own situation is the cause (e.g., client is traveling, Client\'s old maid returning, client no longer needs, client is switching preferences).
- If the client’s complaint is about the maid (such as her performance, skills, health, or behavior ... etc), do not classify as Client-Related Reasons — instead, map to the appropriate category.
- Do not classify temporary administrative issues (refunds, delays, system errors, transparency problems, miscommunication with the company) as root causes unless they are clearly the reason the maid was replaced.
- If both maid-related and client-related causes are mentioned, always include both categories and subcategories. Never drop one in favor of the other. Always list maid-related causes first, followed by client-related causes.
- If multiple reasons exist (maid-related, client-related, or both), include all accurate themes corresponding to the summary, but always list the strongest/most emphasized root cause first.

Never treat the following as root causes:
refunds, credits, payment processing steps, admin delays, staff names, logistics, system bugs, or contract procedures.
These are outcomes or processes, not dissatisfaction causes. Ignore them.

Reasoning Rule:
Think step by step to decide the classification.
- First, identify all dissatisfaction causes.
- Then, filter out outcomes and admin details.
- Finally, map each cause to its correct broad category and subcategory.
Do this reasoning silently. Do NOT include reasoning in your final output.

Output ONLY in this format (no extra words):
all_case_themes: <comma-separated list>
subcategory_themes: <comma-separated list>
"""
...
if uploaded_file:
    ...
    master_df = df.copy()
    # Create tabs
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["Matching Scores", "Optimal Matches","Customer Interface", "Maid Profile Explorer", "Summary Metrics","Complaint Deduplication"])
    ...
    # ---------------- Tab 6: Complaint Deduplication & Classification ----------------
    with tab6:
        st.subheader("🧹 Deduplicate Maids & Merge Complaints")
    
        if "maid_id" in df.columns and "complaint_summary" in df.columns:
            # Step 1: Keep only relevant columns
            complaint_df = df[["maid_id", "complaint_summary"]].copy()
    
            # Step 2: Clean complaint summaries (remove NaN)
            complaint_df["complaint_summary"] = complaint_df["complaint_summary"].fillna("")
    
            # Step 3: Group by maid_id and merge all complaints into one
            merged_complaints = (
                complaint_df.groupby("maid_id", as_index=False)
                .agg({"complaint_summary": lambda x: " | ".join([c for c in x if c.strip() != ""])})
            )
    
            # Step 4: Display results
            st.markdown("### ✅ Deduplicated Complaint Summary per Maid")
            st.dataframe(merged_complaints)
    
            st.success(f"✅ Deduplication complete: {len(merged_complaints)} unique maids.")
        else:
            st.error("❌ Columns 'maid_id' and 'complaint_summary' were not found in the dataset.")
            st.stop()
    
        # -------------------------------
        # Step 6: Classify complaints with Gemini
        # -------------------------------
        st.markdown("### 🤖 Classify Complaints with Gemini")
        st.info("This step will analyze and classify up to 100 maids' complaints using the Gemini API.")
    
        if st.button("Run Complaint Classification"):
            model = genai.GenerativeModel("gemini-1.5-pro")
    
            # Limit to first 100 maids for testing
            merged_complaints = merged_complaints.head(100)
            total = len(merged_complaints)
    
            # Create containers for results
            all_themes, sub_themes, summaries, evidences = [], [], [], []
    
            # Progress tracking
            progress = st.progress(0)
            status_text = st.empty()
    
            # Iterate over each maid and classify once
            for i, row in merged_complaints.iterrows():
                text = row["complaint_summary"]
                maid_id = row["maid_id"]
                status_text.text(f"Processing maid {maid_id} ({i+1}/{total})...")
    ...
    Summary: <short rewritten summary>
    All Case Themes: <comma-separated list>
    Subcategory Themes: <comma-separated list>
    Evidence: <bullet list of direct quotes>"""
    
                try:
                    response = model.generate_content(prompt)
                    output = response.text.strip()
    
                    # Extract structured parts from Gemini output
                    summary_match = re.search(r"Summary:\s*(.*)", output)
                    themes_match = re.search(r"All Case Themes:\s*(.*)", output)
                    subthemes_match = re.search(r"Subcategory Themes:\s*(.*)", output)
                    evidence_match = re.search(r"Evidence:\s*((?:.|\n)*)", output)
    
                    summaries.append(summary_match.group(1) if summary_match else "")
                    all_themes.append(themes_match.group(1) if themes_match else "")
                    sub_themes.append(subthemes_match.group(1) if subthemes_match else "")
                    evidences.append(evidence_match.group(1).strip() if evidence_match else "")
                    ...
                    sub_themes.append("")
                    evidences.append("")
                    st.warning(f"⚠️ Error with maid {maid_id}: {e}")
                    print(f"DEBUG {maid_id} | {text[:100]} | Error: {e}")
    
                # Save progress every 20 records
                if (i + 1) % 20 == 0:
                    checkpoint_path = f"checkpoint_partial_{i+1}.csv"
                    merged_complaints.iloc[:i+1].assign(
                        summary=summaries,
                        all_case_themes=all_themes,
                        subcategory_themes=sub_themes,
                        evidence=evidences,
            ...
            merged_complaints["summary"] = summaries
            merged_complaints["all_case_themes"] = all_themes
            merged_complaints["subcategory_themes"] = sub_themes
            merged_complaints["evidence"] = evidences
    
            # Show results
            st.markdown("### 🧩 Classified Complaints (Top 100)")
            st.dataframe(merged_complaints)
    
            # Download enriched dataset
            st.download_button(
                label="Download Classified Complaints CSV",
                data=merged_complaints.to_csv(index=False).encode("utf-8"),
                file_name="classified_complaints.csv",
                mime="text/csv",
            )
    
            st.success("✅ Complaint classification complete and saved!")
